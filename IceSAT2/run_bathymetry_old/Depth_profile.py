...
def apply_DBSCAN(df, out_path, is2, laser, eps_val, min_samp):
#def apply_DBSCAN(df, out_path, is2, laser):

    empty_df = pd.DataFrame()

    #set missing parameter values
    #eps_val = 1
    #min_samp = 5
    ...
    df = df.loc[df.Height < water_level_thresh]
    ...
    df['labels'] = labels
    reef_depths = df.loc[df.labels >= 0]
    #if len(reef_ph) < 1000:
    #    return empty_df
