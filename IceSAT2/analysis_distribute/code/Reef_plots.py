...
#method to plot sentinel histogram of just the training 
    ...
    plt.setp(ax, xlim = [0, 3000], ylim = [0, 3000])
...
def plot_reefs(reef_depths, training_data, sf, line, reef):
#method to plot the reef depth histogram and a scatter plot of the same
    ...
    #plot reef depths
    reef_scatter(fig, ax[2], reef_depths, training_data, sf, reef)
...
def line_of_best_fit(ax, training_data, sf, line):
    ...
    sns.scatterplot(x = training_data['diff_b2_b3'], y = training_data.Height, color = 'green', ax = ax)
...
def reef_scatter(fig, ax, reef_depths, training_data, safe_file, reef, suffix = ''):
...
def plot_sentinel_cal(data, slope1, intercept1, slope2, intercept2):
...
def plot_sentinel_icesat(imgs, meta, df):
    ...
    photon_y = [Item[0] for Item in df.image_coords]
    photon_x = [Item[1] for Item in df.image_coords]
