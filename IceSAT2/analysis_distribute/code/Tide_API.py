...
def get_tide(coords, dt, reef):
    ...
    query_string = '&lat={lat}&lon={lon}&start={ut}&length=1000&key={api_key}'\
                        .format(lat = lat,lon = lon,ut = ut, api_key = get_API_key(reef))
...
def get_API_key(reef):
    """
    Loading in API ket from data params
    Return - str - API key
    """
    #loads in data params and returns API key
    json_fp = os.path.join(reef.get_metadata_dir(), reef.get_reef_name() + '_params.json')
    params_f = open(json_fp)
