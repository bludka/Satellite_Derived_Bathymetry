...
def prep_df(sf,fp,crs):
    ...
    print(is2_dt)
    is2_tide_level = tide.get_tide(sf.bbox_coords, is2_dt)
    print('ICESat-2 tide level: {}'.format(is2_tide_level))
    print('Sentinel-2 tide level: {}'.format(sf.get_tide()))
    print('')
...
def remove_log_outliers(data):
    ...
    while curr > min(data['Height']):
        ...
        if len(temp) >= 1:
            ...
            #print(curr, len(temp), lower_outlier, upper_outlier)
        #else:
            #print(curr, len(temp))
        curr -= intervals
    return new_data
