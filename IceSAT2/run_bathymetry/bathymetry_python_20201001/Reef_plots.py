...
def plot_sentinel_cal(data, line_slope, line_intercept):
    ...
    plt.scatter(data.Height, np.log(data.b2), marker='.', color = 'blue')  
    plt.scatter(data.Height, np.log(data.b3), marker='.', color = 'green')  
    plt.scatter(data.Height, np.log(data.b4), marker='.', color = 'red')  

    #Log(Band) against each other
    ax = fig.add_subplot(2, 2, 2)
    plt.title('Comparison of log intensity of Blue vs Green Bands')
    ax.set_xlabel('Blue', fontsize=13)
    ax.set_ylabel('Green', fontsize=13)
    plt.setp(ax, xlim = [5, 8], ylim = [5, 8])
    plt.scatter(np.log(data.b2), np.log(data.b3), color = 'blue')
    scale = 1.00
    ...
    plt.title('Log band difference Blue - Green')
    plt.scatter(data.Height, np.log(data.b2) - np.log(data.b3), marker='.', color = 'aqua')  
