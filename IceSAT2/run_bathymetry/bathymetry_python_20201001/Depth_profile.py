...
def convert_h5_to_csv(is2_file,laser,out_fp):
    ...
    if len(df) != 0:
        ...
        #adjusting for sea level, speed of light in water, tide	
...
def apply_DBSCAN(df, eps_val, min_samp, water_thresh_low, water_thresh_high):
    ...
    df = df.loc[(df.Height > water_thresh_low) & (df.Height < water_thresh_high)]

    if len(df) == 0:
        return empty_df, empty_df
    #get sea level (NO LONGER NEED, SINCE HEIGHTS ARE CORRECTED FOR THIS in water_leve.nomalise_sea_level())
    #sea = df.sea_surface - np.mean(df.sea_surface)
...
def process_h5(reef, is2_file):
    ...
    for laser in is2_file.get_strong_lasers():
        ...
        surface, non_surface = get_surface_photons(photons)
        eps_val = 1
        min_samp = 5
        depth, unclassified = apply_DBSCAN(non_surface, eps_val, min_samp)
        depth.to_csv(processed_output_path)
