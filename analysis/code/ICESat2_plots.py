...
def plot_is2_depths_bathy(photons, is2, laser, depths, images_fp, f, bounds, titleprefix):
    ...
    #plot photons
    plt.scatter(photons.Latitude, photons.Height, marker='o', s = 2, color = 'black')
    
    #plot depths
    #plt.xlim(min(depths.Latitude), max(depths.Latitude))
    plt.scatter(depths.Latitude, depths.Height, s = 2, color = 'orange') #, linestyle='-')
    #plt.plot(depths.Latitude, depths.Height, linestyle='-', color='orange', linewidth=0.5, markersize=0.1)

    #plot surface
    #plt.plot(surface.Latitude, surface.Height, linestyle='-', marker='o',color='blue', linewidth=1, markersize=2,alpha = 0.4)
    ...
    if f != 'False':
        ...
        sea_level = f(sea_lats)
        plt.scatter(sea_lats, sea_level, s= 0.1, color = 'yellow')
        plt.plot(sea_lats, sea_level, linestyle='-', marker='o', color='yellow', linewidth=1, markersize=2)

    plt.savefig(images_fp + '/photon_preds_' + str(date.date()) + '_' + laser + '.png', dpi=300)
