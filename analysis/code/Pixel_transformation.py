...
def prep_df(sf,fp,crs,reef):
    ...
    """    
    #reading in the processed IS2 depths
    df = pd.read_csv(fp)
    
    #getting tide for the IS2 pass
    is2_fn = os.path.basename(fp)
    is2_dt = re.findall('\d{14}', is2_fn)[0]
    is2_dt = datetime.strptime(is2_dt, '%Y%m%d%H%M%S')   
    is2_tide_level = tide.get_tide(sf.bbox_coords, is2_dt, reef)
    print('{0}   S2 tide: {1:7.4f}  IS2 tide: {2:7.4f}'.format(is2_dt, sf.get_tide(), is2_tide_level))

    #adjusting the heights based on the tide on the given day
    df.Height = df.Height - is2_tide_level + sf.get_tide()

    #converts the lat and lon to the crs of the image
    df['Coordinates'] = list(zip(df.Longitude, df.Latitude))
    df['Coordinates'] = df.Coordinates.apply(Point)
    df = gpd.GeoDataFrame(df, geometry = 'Coordinates')
    df.crs = "epsg:4326"  #IS2 is in WGS84 Coordinates
    df = df.to_crs(crs)
    return df
...
def load_ICESAT_predictions(icesat_proc_path,sf):
    ...
    for fn in is2files:
        ...
        train = pd.concat([train, prep_df(sf,train_path,sf.get_crs(),reef)])
    return train
...
def get_regressor(reef,sf):
    ...
    icesat_proc_path = reef.get_icesat_bathymetry_path()
...
def get_regressor_new(reef,sf):
    ...
    #cleans data from outliers and performs linear fit
    train_data_cleaned = remove_log_outliers(train)
    x = train_data_cleaned.loc[:,['Height']]
    y = train_data_cleaned.loc[:,['diff']]
    m, c = np.polyfit(np.ravel(x), np.ravel(y), 1)
...
def remove_log_outliers(data):
    ...
    while curr > min(data['Height']):
        ...
        if len(temp) >= 1:
            q1, q3 = np.percentile(temp['diff_b2_b3'],[25,75])
            iqr = q3-q1
            outlier_range = 1.5 * iqr
            lower_outlier = q1 - outlier_range
            upper_outlier = q3 + outlier_range
            drop_indices = temp.loc[(temp['diff_b2_b3'] < lower_outlier) | (temp['diff_b2_b3'] > upper_outlier)].index
            new_data = new_data.drop(drop_indices)
        curr -= intervals
    return new_data

def remove_log_outliers_raw(data):
    """
    Removes outliers for training data
    Params - 1. data (DataFrame) - training data (pixels and depth predictions
    Return - DataFrame - removed outlieres at 0.5m intervals
    ...
    while curr > min(data['Height']):
        temp = new_data.loc[(new_data['Height'] < curr) & (new_data['Height'] > curr - intervals)]
        if len(temp) >= 1:
            #run test on blue band only
            log_b2 = temp.apply(lambda x: math.log(x['b2']), axis = 1)
            q1, q3 = np.percentile(log_b2,[25,75])
            iqr = q3-q1
            outlier_range = 1.5 * iqr
            lower_outlier = q1 - outlier_range
            upper_outlier = q3 + outlier_range
            drop_indices_b2 = log_b2.loc[(log_b2 < lower_outlier) | (log_b2 > upper_outlier)].index
            #run test on green band only
            log_b3 = temp.apply(lambda x: math.log(x['b3']), axis = 1)
            q1, q3 = np.percentile(log_b3,[25,75])
            iqr = q3-q1
            outlier_range = 1.5 * iqr
            lower_outlier = q1 - outlier_range
            upper_outlier = q3 + outlier_range           
            drop_indices_b3 = log_b3.loc[(log_b3 < lower_outlier) | (log_b3 > upper_outlier)].index
            #edit
            idrop_b2 = np.array(drop_indices_b2)
            idrop_b3 = np.array(drop_indices_b3)
            idrop = np.concatenate((idrop_b2, idrop_b3))
            drop_indices_all = np.unique(idrop)
            new_data = new_data.drop(drop_indices_all)
        curr -= intervals
    return new_data
