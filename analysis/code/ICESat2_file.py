...
class IS2_file():
    ...
    def __init__(self, reef, h5_fn):
        ...
        self.reef_dir = reef.get_reef_dir()
        self.metadata_dir = reef.get_metadata_dir()
        self.bbox_coordinates = reef.get_bounding_box()	#IS2 bounding box and reef bounding box are the same
        self.h5_dir = reef.get_icesat_rawdata_path()
        self.h5_fn = h5_fn

        #create h5 file path
        self.h5_file = os.path.join(self.h5_dir, self.h5_fn)

        #get orientation of satellite and choosing strong lasers accordingly
    ...
    def get_orientation(self):
        """
        Extract the oriesntation of the satelitte 
    ...
    def load_json(self):
        ...
        """
        metadata_path = os.path.join(self.metadata_dir, 'ICESAT_metadata.json')
    ...
    def get_photon_data(self, laser):
        ...
        #return list of photon height, lat,lon and photon confidence
        if laser + '/heights/' in h5.keys():
            ...
            return [height,lat,lon,conf_land,conf_ocean,conf_inlandwater]
        else:
            return []
